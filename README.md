...
Consult the [source code](./src/Faithlife.FakeData) for additional details.
...
Some attributes from [`System.ComponentModel.DataAnnotations`](https://learn.microsoft.com/dotnet/api/system.componentmodel.dataannotations) are also supported:

* [`KeyAttribute`](https://learn.microsoft.com/dotnet/api/system.componentmodel.dataannotations.keyattribute): Indicates the primary key. If the column type is `int` or `long`, it defaults to an "auto-increment" value when a row is added.
* [`RequiredAttribute`](https://learn.microsoft.com/dotnet/api/system.componentmodel.dataannotations.requiredattribute): Prevents the column from being set to `null`.
* [`StringLengthAttribute`](https://learn.microsoft.com/dotnet/api/system.componentmodel.dataannotations.stringlengthattribute): Prevents the column from being set to a string longer than the specified length.
* [`RegularExpressionAttribute`](https://learn.microsoft.com/dotnet/api/system.componentmodel.dataannotations.regularexpressionattribute): Prevents the column from being set to a string that doesn't match the specified regular expression.
...
The database schema is represented by a class that derives from [FakeDatabaseContext](./src/Faithlife.FakeData/FakeDatabaseContext.cs). It should have one read-only property of type [FakeDatabaseTable&lt;T&gt;](./src/Faithlife.FakeData/FakeDatabaseTable.cs) for each table in the database. Initialize each property in the constructor with a call to [CreateTable&lt;T&gt;()](./src/Faithlife.FakeData/FakeDatabaseContext.cs).
...
To create an empty database, call [FakeDatabase.Create&lt;T&gt;()](./src/Faithlife.FakeData/FakeDatabase.cs).

```csharp
var database = FakeDatabase.Create<UserDatabaseContext>();
```

To add rows to a table, lock the database and call [Add](./src/Faithlife.FakeData/FakeDatabaseTable.cs) or [AddRange](./src/Faithlife.FakeData/FakeDatabaseTable.cs) on the table from the returned context.
...
To update rows in a table, call [UpdateWhere](./src/Faithlife.FakeData/FakeDatabaseTable.cs), which returns the number of rows affected.

```csharp
context.Users.UpdateWhere(x => x.UserId == bob.UserId, x => x.Name = "Robert");
```

To remove rows from a table, call [RemoveWhere](./src/Faithlife.FakeData/FakeDatabaseTable.cs), which also returns the number of rows affected.
